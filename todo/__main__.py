# SCTRANSLIT -- Special Characters and Transliteration replacement with MuleStrings.
# Copyright (C) 2024  Immanuel Stemmert (Bertrandterrier) 
#
...
from path import search_correct_path
import pandas as pd
import os
import argparse
import sys
    ...
    def _extract_search_command(self,command:str) -> tabulate:
        ...
        for row in range(self.df.shape[0]):
            for col in cols:
                ...
                if not self.df[col].dtypes in ['str','object']:
                    self.df[col] = self.df[col].astype('str')
                
                keywords:list[str] = [elem for elem in self.df[col].unique()]
                for key in keywords:
...
def main():
   ...
   # Check if settings file is existing
    ...
    if not args.set == None:
        ...
        path:str = args.input.strip()
        print(f'"{args.input}"')
        print(f'"{path}"')
    elif not args.create == None:
        path:str = os.path.expanduser(args.create.strip())
        dest:str = os.path.dirname(path)

        btp('Copy CSV template to path',f'::{path}')
        
        if not os.path.exists(dest):
            old_dest = dest
            dest = search_correct_path(dest)
        
        if os.path.exists(dest):
            path = path.replace(old_dest,dest)
            shutil.copy2(CSV_TMPL_PATH,path)
        else:
            btp('WARNING',f':::Did not found PATH {path} or similar paths.',':::Aborting...')
            sys.exit(3)
    elif def_file == None:
    ...
    while exit_check == False:
        ...
        if opt.lower().startswith("w") or opt.lower().endswith("w"):
            if not "w:" in opt.lower():
                ...
                if opt.lower()=='w:table':
                    fname = opt.replace('w:table','')
                    ...
                    tbl_path:str = os.path.join(os.path.dirname(path),fname)
                print('TESTTESTTEST')
                print(todolist.table)
                print(f"{todolist.table}")
                with open(tbl_path,"w") as f:
                    f.write(f'{todolist.table}')
                with open(tbl_path+'_all','w') as f:
                    f.write(todolist.table_all)
                with open(tbl_path+'_fin','w') as f:
                    f.write(todolist.table_fin)
